#!/usr/bin/env python3
"""v3: Line-by-line replacement of Chinese string literals with l10n.get() calls."""
import re, os

BASE = '/Users/yingjunchi/code/mianshi-zhilian-app/apps/client/lib'

# All Chinese -> l10n key mappings (sorted by length desc for priority)
STRING_MAP = {
...
# Patterns that indicate a string literal context where we should NOT replace
SKIP_CONTEXTS = [
    r'import\s',  # import statements
    r'//',  # comments (before the match)
    r'\.get\(',  # already using .get()
    r'key\s*:',  # map key assignments like 'key': value
]

def is_in_string_context(line, pos):
    """Check if position is inside a string that shouldn't be replaced."""
    before = line[:pos]
    # Check if inside an import
    if 'import ' in before and "'" in before:
        return True
...
def replace_line(line, cn, key):
    """Replace a Chinese string literal on a single line."""
    replacement = f"l10n.get('{key}')"

    # Skip if already replaced
    if f"l10n.get('{key}')" in line:
        return line
    ...
    # Pattern: 'Chinese text' (single quotes)
    # We need to be careful about context
    old = f"'{cn}'"
    if old in line:
        # Check context - don't replace in map definitions, variable assignments, etc.
        # Only replace in widget tree contexts (Text, hintText, labelText, etc.)
        idx = line.index(old)
        before = line[:idx].rstrip()

        # Replace const Text(...) -> Text(...)
        if f"const Text({old})" in line:
            line = line.replace(f"const Text({old})", f"Text({replacement})")
        elif f"Text({old})" in line:
            line = line.replace(f"Text({old})", f"Text({replacement})")
        elif f"Text(\n" in line or f"Text(" in before:
            # Multiline Text - the string is likely the first arg
            line = line.replace(old, replacement, 1)
        elif 'hintText:' in before:
            line = line.replace(old, replacement, 1)
        elif 'labelText:' in before:
            line = line.replace(old, replacement, 1)
        elif 'tooltip:' in before:
            line = line.replace(old, replacement, 1)
        elif 'title:' in before and ('Text(' in line or 'const Text(' in line):
            pass  # handled by Text pattern
        elif 'content:' in before and ('Text(' in line or 'const Text(' in line):
            pass
        elif 'label:' in before and ('Text(' in line or 'const Text(' in line):
            pass
        # For standalone strings in widget tree (e.g., in a list of strings for UI)
        # We check if the line is indented (likely in a widget tree)
        elif len(line) - len(line.lstrip()) >= 6:  # deeply indented = likely widget tree
            line = line.replace(old, replacement, 1)
...
def add_l10n_import(content):
    """Add localization_provider import after last provider import."""
...
def add_l10n_variable(content):
    ...
    # Find the last context.watch line and add after it
    lines = content.split('\n')
    new_lines = []
    last_watch_idx = -1
    for i, line in enumerate(lines):
...
def process_file(filepath):
    ...
    # Step 2: Replace Chinese strings line by line
    lines = content.split('\n')
    new_lines = []
    for line in lines:
        for cn, key in STRING_MAP.items():
            if cn in line and f"l10n.get('{key}')" not in line:
                line = replace_line(line, cn, key)
        new_lines.append(line)
    content = '\n'.join(new_lines)
    ...
    for f in files:
        if os.path.exists(f):
            ...
            with open(f, 'r') as fh:
                ...
                # Count remaining Chinese strings in non-comment, non-import lines
                remaining = 0
                for line in c.split('\n'):
                    s = line.lstrip()
