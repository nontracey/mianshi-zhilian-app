#!/usr/bin/env python3
"""Batch-apply l10n.get() calls to replace hardcoded Chinese strings."""
import re, sys, os

# Mapping: Chinese string -> l10n key
STRING_MAP = {
    # header_bar
    '搜索结果': 'search_results',
    ...
    # interview_prep_page
    '面试准备': 'interview_prep_generic',  # partial match, will handle context
    '调整目标': 'adjust_target',
    '设置目标': 'set_target',
    ...
    # profile (partial - many more needed)
...
# Parameterized strings (Chinese template -> (key, param extraction))
PARAM_MAP = {
    '第 {n} 轮': ('round_number', lambda m: {'number': m.group(1)}),
    '问题 {n}': ('question_number', lambda m: {'number': m.group(1)}),
}


def needs_l10n_import(content):
    return "import 'package:mianshi_zhilian/providers/localization_provider.dart';" not in content


def add_l10n_import(content):
    """Add localization_provider import after the last provider import."""
    import_line = "import 'package:mianshi_zhilian/providers/localization_provider.dart';"
    if import_line in content:
        return content

    # Find a good place to insert - after last provider import or after last import
    lines = content.split('\n')
    insert_idx = 0
...
def process_file(filepath):
    ...
    if needs_l10n_import(content) and any(cn in content for cn in STRING_MAP):
        content = add_l10n_import(content)

    # Step 2: Replace strings in Text() widgets and similar contexts
    # Pattern: 'Chinese text' inside Text(), const Text(), etc.
    # We need to be careful to only replace string literals that are UI text

    # For simple replacements in const Text('...') and Text('...')
    for cn, key in sorted(STRING_MAP.items(), key=lambda x: -len(x[0])):
        # Skip very short strings that might cause false positives
        if len(cn) <= 1:
            continue

        # Replace in specific patterns to avoid false positives
        # Pattern 1: const Text('中文') -> Text(l10n.get('key'))
        # Note: removing const because l10n.get() is not const
        ...
        # Pattern 2: Text('中文') -> Text(l10n.get('key'))
        # But not if already replaced
        content = content.replace(
            f"Text('{cn}')",
            f"Text(l10n.get('{key}'))"
        )
        ...
        # Pattern 3: SnackBar(content: Text('中文'))
        # Already handled by Pattern 2

        # Pattern 4: hintText: '中文'
        content = content.replace(
            f"hintText: '{cn}'",
            f"hintText: l10n.get('{key}')"
        )
        content = content.replace(
            f'hintText: "{cn}"',
            f"hintText: l10n.get('{key}')"
        ...
        content = content.replace(
            f"labelText: '{cn}'",
            f"labelText: l10n.get('{key}')"
        )

        # Pattern 6: title: '中文' (in AppBar, etc.) but not in maps
        # Only replace in specific contexts

        # Pattern 7: tooltip: '中文'
        content = content.replace(
            f"tooltip: '{cn}'",
            f"tooltip: l10n.get('{key}')"
        )
...
def main():
    ...
    for f in files:
        if os.path.exists(f):
            ...
            print(f"{'✅' if changed else '⏭️'} {os.path.basename(f)}")
        else:
            print(f"❌ Not found: {f}")
