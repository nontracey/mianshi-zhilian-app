...
class ContentApiService {
  ...
  Future<Domain> fetchDomain(String domainId) async {
    final response = await http.get(Uri.parse('$baseUrl/domains/$domainId.json'));
    if (response.statusCode == 200) {
      return Domain.fromJson(json.decode(response.body) as Map<String, dynamic>);
    }
  ...
  Future<Topic> fetchTopic(String topicPath) async {
    final response = await http.get(Uri.parse('$baseUrl/topics/$topicPath.json'));
    if (response.statusCode == 200) {
