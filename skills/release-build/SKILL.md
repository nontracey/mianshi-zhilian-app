...
```
检测变更 → 创建发布分支 → 提交功能变更(feat/fix) + 递增构建号(chore) → 推送分支 → auto-release.sh 自动创建 PR + auto-merge + CI 等待 + 强制重打 tag
```
...
该脚本自动完成：创建 PR → 启用 auto-merge → 等待 CI 通过后自动合并 → 切回 main 同步 → 强制重打当前版本 tag 并推送。
