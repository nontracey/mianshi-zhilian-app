...
```
检测变更 → 创建发布分支 → 提交功能变更(feat/fix) + 递增版本号+构建号(chore) → 推送分支 → auto-release.sh 自动创建 PR + auto-merge + CI 等待 + 打新 tag
```
...
bash scripts/auto-release.sh version {newVersion} {buildNumber}
```

该脚本自动完成：创建 PR → 启用 auto-merge → 等待 CI 通过后自动合并 → 切回 main 同步 → 打新 tag 并推送。
