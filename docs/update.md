...
- **应用内不中断**：下载由全局 `UpdateDownloadProvider` 驱动而非绑定在进度对话框上，应用内切换页面不会中断下载，进度在更新页内联展示（注意这不是 OS 级后台下载，App 被系统挂起仍会停）。
- **已下载可再次安装**：下载并校验通过的安装包路径与版本会持久化（`downloaded_installer`），更新页常驻「已下载 vX.Y，点此安装」入口；即便没有立即安装、之后也能再次打开安装，或一键删除安装包。该入口与「是否有新版本」无关，只要存在有效已下载记录就展示。
- **清理旧安装包**：下载成功后会清理临时目录里其它版本的 `mianshi-zhilian-v*` 安装包，避免跨版本累积占用空间。
