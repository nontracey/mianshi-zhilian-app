...
当前应用采用本地优先模式。用户学习、练习和配置数据先写入本机存储，之后按同步设置导出、推送或拉取；AI、内容接口和账号类功能则按需即时请求远端服务。

## 本地优先并参与同步的数据

这些数据写入本地 `SharedPreferences`。当同步方式配置为 WebDAV、GitHub 或 Gitee 时，会被打包成同步快照；本地文件导入导出也使用同一批业务数据。
...
- 默认不上传完整练习回答文本，`practice_attempts.answer` 会清空，`improvedAnswer` 会置空。
