...
- `ai_configs`：AI 配置元数据。默认不进入同步快照；只有用户显式开启“同步 AI 配置元数据”时才同步，且永远不包含 API Key。
...
- 默认同步备考目标和项目库；这些数据可能包含公司、JD、岗位、项目细节，用户可手动关闭同步。
- `local_profile` 中的头像（`avatarUrl`）始终同步，不受隐私开关限制；仅剥离邮箱和账号绑定状态。
