...
4. 更新检查使用 `RouteResolver.appApi + /update.json`，由客户端自动在 `https://mianshi-zhilian-api.pages.dev` 与 `https://mianshizhilian-api.nontracey.de5.net` 间选择。生产构建不再使用 `API_BASE_URL` / `UPDATE_MANIFEST_URL`。下载安装包时优先尝试官方 Web 主备镜像路径，再尝试 GitHub 官方、用户自定义镜像和 ghfast.top；用户可在 App 的"关于与更新 → ⚙️ 下载设置"中配置自定义镜像站前缀。
