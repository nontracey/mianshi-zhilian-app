...
> 注：上表为最初视觉方向。当前实现的默认主题色与设计稿略有差异——`ThemeProvider` 默认主色为 `#1A2B4A`、强调色为 `#3078F0`，以代码默认值为准。
...
早期曾评估“微信小程序 + H5”路线；当前已落地为 **Flutter App + Web** 主路线。以下 3.1 仅保留历史方案比选，后续维护以 3.2 的 App 优先方案为准。

### 3.1 小程序优先方案（已废弃，仅留档）

> 本节为早期方案比选记录。项目最终采用 3.2 的 Flutter App 优先方案，微信小程序未实施，本节内容不再维护。
...
| 历史迁移 | 当时认为从 H5 迁移到 App 的成本低于纯原生小程序 |
...
| 历史小程序方案 | 未实施，不作为当前主路线 |

### 3.3 多端方案对比（历史选型，已废弃，仅留档）

> 本节记录早期技术选型过程。当前事实路线为 Flutter App + Web；含小程序的候选方案不再作为后续计划维护。

| 方案 | 优点 | 缺点 | 结论 |
| --- | --- | --- | --- |
| 微信原生小程序 | 微信能力最稳 | 电脑端 H5 需要另做一套 | 已废弃，仅留档 |
| Taro + React | 多端能力强 | 项目心智成本略高 | 已废弃，仅留档 |
| uni-app + Vue 3 | 小程序/H5 成本低，生态成熟 | 原生桌面端不是强项，自定义 URL 仍受小程序限制 | 已废弃，仅留档 |
...
| Next.js H5 + 小程序另做 | Web 体验强 | 双端成本高 | 已废弃，仅留档 |

### 3.4 当前落地路线

当前以 **面试智练 App** 作为主项目，架构上采用：

1. 主客户端：`Flutter`，覆盖 Android、macOS、Windows、Web。
2. 后端：继续用 Cloudflare Worker + D1/KV/R2。
3. 知识内容：继续 Git + Cloudflare Pages/R2 静态分发。
4. AI 请求：App 端优先直连用户自定义 URL；Web 端按 CORS 情况走 Worker 代理。

## 4. 总体架构

```mermaid
flowchart TD
  A["客户端<br/>Flutter App / Web"] --> B["本地缓存<br/>SharedPreferences / secure storage"]
  A --> C["Cloudflare Worker API"]
  C --> E["用户数据存储<br/>Cloudflare D1"]
...
4. Android、macOS、Windows App 优先直连 AI；Web 根据 CORS/域名限制走代理。
...
| GitHub Pages + LocalStorage | 0 成本 | H5 原型 | 账号与跨端能力弱 |
| 微信云开发免费额度 | 低 | 历史小程序方案 | 已废弃，仅留档 |
| Supabase 免费额度 | 低 | 需要管理后台和 SQL 能力 | 免费额度和网络稳定性需观察 |
| Cloudflare D1 + Worker | 低 | Web + App | Web 仍有 CORS 问题 |
...
当前主路线是 App + Web，登录不绑定微信。实际已落地用户名密码 + 刷新令牌登录，游客模式仍为默认可用入口。
...
| 微信登录 | 历史小程序方案 | 微信生态方便 | 桌面/Web 统一性一般 | 已废弃，仅留档 |
| 手机号登录 | 全端 | 用户熟悉 | 短信成本和合规成本 | 暂缓 |

### 6.1 微信登录设计（已废弃，仅留档）

> 本节为小程序方案配套的登录设计，未实施。当前实际登录方案为用户名密码 + 刷新令牌（见 Worker API），游客模式仍为默认。
...
| 主题/语言设置 | 登录设备本地设置优先 |
| AI 配置/API Key | 默认不跨端同步；用户显式开启后只同步配置元数据，不同步 API Key；如需同步密钥，必须端到端加密 |
...
| 页面 | 功能 |
| --- | --- |
| 学习首页 | 展示当前学习范围（全领域/单领域/路线）、对应掌握度、范围选择入口和继续学习入口 |
| 领域知识目录 | 点击 Java、Agent 开发、算法领域卡片后进入，查看该领域下的所有知识内容 |
| 知识目录 | 按当前学习范围（领域/路线）展示知识分类和知识点，可筛选熟练/不熟练/未掌握 |
...
5. 切换学习范围时，保留其他领域的学习进度、复习队列和掌握度。
...
第一版练习类型（历史规划。当前已实现 8 种练习模式：今日复习、复述练习、追问训练、薄弱训练、高频冲刺、项目深挖、系统设计、模拟面试，见 `lib/pages/practice/`）：
...
1. 文本输入：第一版必须支持。
2. 语音输入：支持云端 AI 语音、系统语音识别和本机 sherpa-onnx STT；用户触发语音功能时请求麦克风/语音识别权限，拒绝或永久拒绝后提示前往系统设置授权。
...
| 功能 | 说明 |
| --- | --- |
| 账号登录/退出 | 游客模式默认可用；正式账号使用用户名密码 + 刷新令牌 |
| 同步与备份 | 同步方式、目标账号配置、自动同步、同步内容开关、手动同步、恢复、文件导入导出 |
| AI 与语音 | AI 配置入口、默认模型状态、系统语音、本机模型与 ONNX Runtime 资源管理 |
...
学习路线（`LearningRoute`，含 AI 生成与手动自定义）是 **App 侧的用户数据**，但其结构必须严格来自内容库，App 不得另立一套同名领域或自定义阶段顺序：

- 路线的阶段（`phases`）由内容库每个领域已定义的 `learningPaths` 确定性组装（`RouteComposer`，AI 生成与手动编辑共用同一逻辑）。领域内的阶段顺序、分类、知识点顺序全部由内容契约决定。
- 多领域路线 = 所选领域 `learningPath` 的有序拼接；领域顺序由用户/AI 指定，编辑器调整领域顺序时按内容库重建 `phases`，使阶段顺序、目录展示、掌握度统计同步生效。
- 路线「声称的领域」恒等于「实际有内容的领域」：`LearningRoute.effectiveDomainIds` 在有 `phases` 时按 `phases` 推导领域，避免 `domainIds` 与 `phases` 漂移导致目录出现空领域、掌握度只显示部分领域。
- AI 仅负责「选择并排序相关领域」，不再对领域内知识点做二次裁剪；领域选择要求精准（只纳入直接相关与必要前置，排除无关领域）。
- 内容分发分**发布 / 测试 / 草稿**三个环境（`contentEnv`），路线引用的领域、分类、知识点 ID 均与当前环境的内容契约一致。
...
> 注：下表中的存储介质为早期规划。当前实现统一使用 `SharedPreferences`（Web 端为 localStorage）保存业务数据；AI API Key 与记住的登录密码在原生平台写入系统安全存储（Keychain/Keystore/DPAPI），详见 `docs/data-storage-and-sync.md`。

| 数据 | 存储 |
| --- | --- |
| 当前学习进度 | 本地（SharedPreferences） |
| 知识点掌握状态 | 本地 |
| 练习草稿 | 本地 |
| 游客事件队列 | 本地 |
| 待同步队列 | 本地 |
| 登录前进度快照 | 本地 |
| 失败重试记录 | 本地 |
| AI 配置和 Key | 本地；Key 在原生平台写入系统安全存储 |
| 内容 manifest 缓存 | 本地 |
| 最近访问知识点 | 本地 |
...
```json
{
  ...
  "data": {
    ...
    "answer_versions": {},
    "learning_scope": {"kind": "allDomains"},
    "custom_routes": [],
    "selected_route_id": "",
    "disabled_domains": [],
    "settings": {},
...
| --- | --- |
| 知识点进度、分数、复习时间 | 必须同步 |
| 练习记录摘要、练习 session、模拟面试摘要 | 默认同步；完整回答、改写答案、session feedback 默认脱敏 |
| 备考计划、训练计划、目标岗位信息 | 可选同步，默认关闭，可能包含公司、JD、岗位信息 |
| 项目库、项目深挖材料 | 可选同步，默认关闭，可能包含项目经历和业务细节 |
| 回答版本库 | 跟随“同步完整练习回答”，默认不同步 |
| 学习范围（LearningScope：领域/路线/全领域）、自定义路线、禁用领域 | 默认同步 |
| 主题、语言、学习推荐参数、内容源偏好 | 默认同步 |
| AI 配置元数据 | 可选同步，默认关闭；开启后同步名称、Base URL、模型、能力标签，不同步 API Key |

用户可选内容：

| 开关 | 默认 | 说明 |
| --- | --- | --- |
| 同步完整练习回答 | 关 | 关闭时只同步题目、分数、标签、摘要、下一步建议等摘要字段，并脱敏 session feedback |
| 同步项目库和备考目标 | 关 | 可能包含公司、岗位、JD、项目经历 |
| 同步 AI 配置元数据 | 关 | 不包含 API Key；开启后可能暴露私有 Base URL、模型名或网关信息 |
...
| P1 | 微信登录（历史规划） | 小程序方案配套，已废弃留档 | ❌ 不实施 |
...
5. 复核 Web、Android、macOS、Windows 的发布与更新体验。
...
13. 微信小程序不再作为主路线，相关设计仅作为历史留档，不进入阶段计划。
...
sherpa-onnx 模式下用户可选择三种引擎：SenseVoice（多语言 + 情感识别）、Whisper（高精度多语言，tiny/base/small/medium 四种尺寸）、Paraformer（纯中文流式转写）。用户需先下载 ONNX Runtime 和所选模型，两者都就绪后才能使用本机识别。
...
| 资源 | 说明 | 是否可独立管理 |
|------|------|----------------|
| ONNX Runtime | 当前平台和 CPU 架构对应的 sherpa-onnx 原生运行时库 | ✅ |
| 识别模型 | SenseVoice / Whisper / Paraformer 对应的模型文件 | ✅ |

设置页提供“下载所需资源”总入口，也保留 ONNX Runtime 和模型的独立下载、删除入口。下载能力要求：

1. 支持暂停：保留 `.part` 临时文件，下次继续下载时走 HTTP Range 断点续传。
2. 支持取消：中止请求并清理对应资源的未完成临时文件。
3. 支持速度显示：下载中展示百分比、当前源和实时速度。
4. 支持线路自动选择：默认并发探测 GitHub、自定义镜像和 ghfast.top，优先使用响应最快的可达线路。
5. 支持固定线路策略：用户可切换为 GitHub 官方优先、镜像源优先或仅 GitHub 官方。
