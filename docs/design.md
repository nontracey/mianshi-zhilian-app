...
2. 账号同步模式：登录后把学习进度、复习队列、练习记录摘要同步到云端。
...
用户可以先不登录直接学习。未登录期间的学习进度、掌握度、复习队列、练习记录摘要和设置都保存在本地。用户后续登录后，App 必须提示并执行“将本机学习进度同步到账号”。
...
| 练习记录 | 按 `eventId` 去重后合并，默认只同步摘要 |
| 主题/语言设置 | 登录设备本地设置优先 |
| AI 配置/API Key | 不自动上传；需要用户明确开启加密同步 |
...
1. 文本输入：第一版必须支持。
2. 语音输入：可作为增强项，微信小程序可用录音能力，转文字可后续接入。
...
| 学习设置 | 当前学习领域、每日新知识数、每日复习数、推荐策略 |
| 学习统计 | 总知识点、已学、熟练、不熟练、未掌握 |
| AI 配置 | 多模型配置、测试连接、默认模型 |
| 语言设置 | 中文/英文切换 |
| 主题设置 | 主题模式、主色、强调色、字体大小、卡片密度 |
| 数据同步 | 手动同步、查看上次同步时间 |
| 数据导出 | 导出本地学习记录 JSON，防止数据丢失 |
...
| `practice_sessions` | 练习记录摘要，不存完整长文本或仅按需存 |
| `sync_events` | 增量同步事件，可定期归档 |
...
3. 用户练习过程只写本地事件队列，不每题都写云端。
4. 满足以下条件之一再批量同步：
   - 完成一次练习会话。
   - 退出页面。
   - 距离上次同步超过 5 分钟。
   - 用户手动点击同步。
5. 云端保存进度快照，历史详细回答默认只存最近 N 条。
6. 知识内容使用 CDN 缓存和版本号，不查数据库。
...
| 场景 | 用户侧表现 | 产品保护 |
| --- | --- | --- |
| Worker 请求超额或 API 不可用 | 登录、同步、Web AI 代理、检查更新失败 | 本地学习继续可用；新增事件进入待同步队列 |
...
2. 云同步失败不能阻断学习流程。
3. 所有本地学习事件先写入待同步队列，再尝试上传。
4. 同步失败时保留事件并记录失败原因，下次启动、网络恢复、用户手动同步时重试。
5. AI 评估失败不能清空用户输入。
...
8. 用户进度先本地保存，登录后再用 Cloudflare Worker + D1 做增量同步。
